    ...
    <div class="max-w-3xl mx-auto px-4 pt-8 pb-12 lg:pt-12 lg:pb-16">
        ...
        <div class="mb-8 pt-2 lg:pt-0">
            <div class="flex items-center gap-3 mb-3">
                <a href="{{ route('werkbonnen.index') }}" wire:navigate class="w-8 h-8 rounded-full bg-ink-90 border border-ink-70/20 flex items-center justify-center text-paper/50 hover:text-paper hover:border-amber/30 transition shrink-0">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @elseif($entry->type === 'video')
                        <div class="w-6 h-6 rounded-full bg-purple-500/10 flex items-center justify-center">
                            <i class="fa-solid fa-video text-purple-400 text-[9px]"></i>
                        </div>
                        <span class="text-sm font-heading font-semibold text-paper">Video</span>
                    @else
                        ...
                        @foreach($entry->documents as $doc)
                            <div class="flex items-center gap-3 px-5 py-3">
                                <div class="w-8 h-8 rounded-md flex items-center justify-center shrink-0
                                    {{ str_starts_with($doc->mime_type ?? '', 'image/') ? 'bg-blue-500/10' : (str_starts_with($doc->mime_type ?? '', 'audio/') ? 'bg-amber/10' : (str_starts_with($doc->mime_type ?? '', 'video/') ? 'bg-purple-500/10' : 'bg-ink-70/15')) }}">
        ...
        <div class="flex items-center gap-3 mt-10 pt-8 border-t border-ink-70/10">
            @if($entry->isDraft())
                <a href="{{ route('werkbonnen.edit', $entry) }}" wire:navigate class="inline-flex items-center gap-2 border border-amber/40 px-5 py-2.5 text-amber font-semibold text-sm font-heading rounded-sm transition hover:bg-amber/10 hover:border-amber/60">
                    <i class="fa-solid fa-pen text-xs"></i>
                    Bewerken
                </a>
                <form action="{{ route('werkbonnen.finalize', $entry) }}" method="POST">
                    @csrf
                    @method('PATCH')
            ...
            @elseif($entry->isFinal() && !$entry->client_id)
                <a href="{{ route('werkbonnen.edit', $entry) }}" wire:navigate class="inline-flex items-center gap-2 border border-amber/30 px-5 py-2.5 text-amber/70 font-semibold text-sm font-heading rounded-sm transition hover:bg-amber/10 hover:text-amber">
                    <i class="fa-solid fa-user-plus text-xs"></i>
                    Koppel klant om te factureren
                </a>
            @endif

            <form action="{{ route('werkbonnen.destroy', $entry) }}" method="POST" onsubmit="return confirm('Weet je zeker dat je deze werkbon wilt verwijderen?')">
                @csrf
                @method('DELETE')
            ...
            <a href="{{ route('werkbonnen.index') }}" wire:navigate class="ml-auto px-4 py-2.5 text-ink-50 font-semibold text-sm font-heading transition hover:text-paper">
