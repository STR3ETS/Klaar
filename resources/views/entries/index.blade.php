    ...
    <div class="max-w-2xl mx-auto px-4 pt-8 pb-12 lg:pt-12 lg:pb-16">
        ...
        <div class="mb-8 pt-2 lg:pt-0">
            ...
            <div class="flex items-center justify-between mt-1">
                <h1 class="font-display text-paper text-2xl lg:text-3xl uppercase">Invoeren<span class="text-amber">.</span></h1>
                <a href="{{ route('entries.create') }}" wire:navigate class="inline-flex items-center gap-2 bg-amber px-4 py-2 text-ink font-semibold text-sm font-heading rounded-sm transition hover:brightness-110 hover:shadow-[0_4px_16px_rgba(255,180,0,0.3)]">
        ...
        <div class="mb-6" x-data="{ filtersOpen: {{ request()->hasAny(['status', 'type', 'client', 'project', 'search']) ? 'true' : 'false' }} }">
            <div class="flex items-center justify-between mb-3">
                ...
                @if(request()->hasAny(['status', 'type', 'client', 'project', 'search']))
                    <a href="{{ route('entries.index') }}" wire:navigate class="text-xs text-ink-50 hover:text-amber transition font-heading">Wis filters</a>
                @endif
            </div>
            ...
            <div x-show="filtersOpen"
                ...
                <form method="GET" action="{{ route('entries.index') }}" class="bg-ink-90 rounded-sm border border-ink-70/20 p-4 space-y-3">
        ...
        @if($entries->isEmpty())
            <div class="text-center py-20">
                ...
                @if(request()->hasAny(['status', 'type', 'client', 'project', 'search']))
                    ...
                    <p class="text-xs text-ink-50 mt-1.5 max-w-xs mx-auto leading-relaxed">Geen invoeren gevonden met deze filters.</p>
                    <a href="{{ route('entries.index') }}" wire:navigate class="mt-6 inline-flex items-center gap-2 border border-ink-70/30 px-5 py-2.5 text-ink-50 font-semibold text-sm font-heading rounded-sm transition hover:bg-ink-90 hover:text-paper">
                        Wis filters
                    </a>
                @else
                    <p class="mt-5 text-sm text-ink-30 font-heading font-semibold">Nog geen invoeren</p>
                    ...
                    <a href="{{ route('entries.create') }}" wire:navigate class="mt-6 inline-flex items-center gap-2 bg-amber px-5 py-2.5 text-ink font-semibold text-sm font-heading rounded-sm transition hover:brightness-110 hover:shadow-[0_4px_16px_rgba(255,180,0,0.3)]">
                        <i class="fa-solid fa-plus text-xs"></i>
                        Eerste invoer
                    </a>
                @endif
        ...
        @else
            <div class="mb-4">
                <span class="text-xs font-heading font-semibold uppercase tracking-wider text-ink-50">Alle invoeren</span>
            </div>
